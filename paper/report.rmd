...
As a result of the COVID-19 pandemic and its large impact across Spain, the monitoring of demographic measures as a direct result of deaths related to such pandemic, as well as similarly future deadly events, has become increasingly important. This project intends to develop a tool to easily monitor a selection of demographic measures, all related to collective deaths of individuals as a result of relevant worldwide events like the one mentioned previously, or perhaps other smaller scale events which could increase mortality in a likely fashion.

The tool consists of an [open-source](https://github.com/dreth/tfm_uc3m) [Shiny-based](https://shiny.rstudio.com/) dashboard (developed in R) where such measures are computed and promptly displayed to simplify the data analysis. The application is containerized using Docker, that enables to make possible to run and use it in any operating system. The data is acquired from the [INE](https://ine.es/) and [Eurostat](https://ec.europa.eu/eurostat/) APIs, freely available on the internet. It is then treated by several Python scripts and stored in a GitHub [repository](https://github.com/dreth/tfm_uc3m_data) where it can be updated anytime during the application's runtime.

The application allows for active, real-time monitoring of mortality and life expectancy of the Spanish population.

The application itself has the following functionality:

- Computation and visualization of several mortality metrics:
    - Excess mortality
...
- Visualization of a map of Spain with the previous metrics per autonomous community (CCAA)

All metrics are calculated weekly with data obtained from INE and Eurostat after an extensive data pre-processing pipeline, starting from the year 2010. The application then reads the treated data and generates 2 different kinds of visualizations for the metric selected (described in the [metrics](#Metrics) section), one is a weekly time series (coloured by year) and a map of Spain where CCAAs are coloured based on a scale. The development was initiated on February 2021 and ended with the first finalized, funcional complete version in August 2021. 

As an open source project, we intend to make it a public good, and anyone can do whatever they want with the original source code as long as they abide to a license. This project is licensed under the [GNU General Public License V3](https://github.com/dreth/tfm_uc3m/blob/main/LICENSE).
...
- To provide an open-source, easy-to-use, web-based, OS-agnostic tool to compute and visualize common mortality-related metrics and life expectancy in time series plots, and static or interactive maps.

    + A big focus of the project was to make this application as simple as possible to use, while maintaining the management of system requirements extremely low. The purpose of such approach is to allow virtually anyone that desires to monitor any of the metrics to do so by running a simple shell command. We want any user or institution with access to a computer and the internet to be able to monitor these metrics, or any such metrics which could be possibly added in the future without much hassle. 

    + An important prerequisite was to make the proyect open-source. This allows anyone to freely use and improve the project by adding extra components like other useful mortality-related measures. This makes the project fully auditable and free to use, modify and extend. 
...
- To provide the user the ability to download the plots and the data (with or without filtering).
...
## 4.1 - Advantages of an open source application over weekly reports in PDF format

There are several advantages an application could potentially have over static, weekly PDF reports like the one metioned in the [previous](#Inspiration) section:
...
- As the application is [open source](https://github.com/dreth/tfm_uc3m), developers could expand the application by adding extra features and contributing directly to the project's development.
...
## 4.2 - Why is monitoring mortality/life expectancy metrics important?
...
- The Government and the Ministry of Health, the main decisionmakers in terms of public health related issues. These two institutions can use these metrics to construct policies, regulations and take measures to protect the general health of the population.
...
# 5 - Metrics computed by the application {#Metrics}
...
$$EM_{w, y} = D_{w, y} - \overline{D_{w, y-5, y}}$$
...
$$EM_{w, y} = D_{w, y} - \frac{1}{n} \sum^{n}_{k=1} D_{w, y-k}$$
...
## 5.2 - Cumulative mortality rate {#CMR}
...
## 5.3 - Cumulative relative mortality rate {#CRMR}
...
## 5.4 - Cumulative mortality improvement factor {#CMIF}
...
To compute $LE$ we must first compute a life table as follows, note that each step also corresponds to a column of the life table in the same order and that summations with no summation variable represent column sums:
...
#### 7.1.1.1 - Death dataset update pipeline
...
#### 7.1.2.1 - Population dataset update pipeline
...
## 7.2 - Data pre-processing pipeline {#PreProcessingPipeline}
...
### 7.2.1 - Pipeline for the deaths dataset {#DeathDatasetPipeline}
...
### 7.2.2 - Pipeline for the population dataset {#PopDatasetPipeline}

The population dataset response JSON came with the following challenges:
...
- The sex also needed to be replaced, as it was in Spanish and therefore wouldn't match the format displayed in the deaths dataset.
...
- The sheer size of the obtained JSON and its somewhat odd formatting compared to the previous one obtained from Eurostat required a large inefficient loop to process. This looping was done using *numpy* arrays, which are often much faster than *pandas* DataFrames, this introduced a few new challenges in order to keep the updating script much more efficient.
...
1. Several dictionaries with reference data and replacements to align the formatting with the deaths dataset are defined. These dictionaries are the following:
...
## 7.3 - Dataset update methodology after pre-processing {#DatasetUpdateMethodology}
...
### 7.5.1 - Mortality {#MortalityLayout}

![Mortality section screenshot](./images/mortality_section.png){width=91%}
...
### 7.5.2 - Maps {#MapsLayout}

![Maps section screenshot](./images/maps_section.png)
...
### 7.5.3 - Life expectancy {#LifeExpLayout}

![Life Expectancy section screenshot](./images/life_expectancy_section.png)
...
### 7.5.4 - Database tables {#DBTablesLayout}

![Database tables section screenshot](./images/db_tables_section.png){width=80%}
...
### 7.5.5 - DB info and update {#DBInfoLayout}

![Database information and update section screenshot](./images/db_info_update_section.png){width=93%}
...
### 7.5.6 - Documentation {#DocsLayout}

![Documentation section screenshot](./images/docs_section.png)
...
genMortPlot <- eventReactive(input$plotMortalityButton, {
    gen_df_and_plot_mortality(
        ...
        age_groups=switch(input$selectAgeGroupsMortalityTotal, 'all'=AGE_GROUPS,
                          'select'=input$selectAgeMortality),
...
## 9.3 - Appendix C: Docker {#AppendixC}
...
## 9.4 - Appendix D: Project repository tree structure {#AppendixD}
...
### 9.4.1 - [**The main project repository** (*dreth/tfm_uc3m*)](https://github.com/dreth/tfm_uc3m)
...
- [**LICENSE**]{color="red"}: This file contains the GNU General Public License V3, as the project is licensed under it.

- [**README.md**]{color="red"}: Readme file explaining how to run the app and some basic information about it.

\newpage

### 9.4.2 - [**The data repository** (*dreth/tfm_uc3m_data*)](https://github.com/dreth/tfm_c3m_data)
...
pdftk "./coverpage/coverpage.pdf" "./coverpage/blankpage.pdf" "./report.pdf" cat output "./final_report.pdf"
```