...
In order to query the Eurostat API, we need to execute several queries using the ```query_eurostat()``` function defined in the [functions.py script, line #23](https://github.com/dreth/tfm_uc3m/blob/main/api/functions.py#L23).
...
4. Each obtained dataset is passed trough the ```generate_death_df()``` function defined in the [functions.py script, line #84](https://github.com/dreth/tfm_uc3m/blob/main/api/functions.py#L84). Thereby generating a partial dataset for the current *age+sex* looping combination.

5. The previously generated partial dataset is added to the ```death_datasets``` list created previously.
